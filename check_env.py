...
class EnvChecker:
    ...
    def _generate_scons_env(self):
        """Generate Linux/WSL environment setup script"""
        ...
        lines.append(f'RTT_EXEC_PATH="{rtt_exec_path}"' if rtt_exec_path else '')
        lines.append(f'STM32_PROG_PATH="{stm32_prog_path}"' if stm32_prog_path else '')
        lines.append(f'STLINK_GDB_SERVER="{stlink_gdb_path}"' if stlink_gdb_path else '')
        lines.append(f'MENUCONFIG_PATH="{menuconfig_path}"' if menuconfig_path else '')
        lines.append(f'ENV_TOOLS_DIR="{env_tools_dir}"' if env_tools_dir else '')
        lines.append(f'RTT_CC="gcc"')
        lines.append(f'RTT_EXEC_PATH="{rtt_exec_path}"' if rtt_exec_path else '')
        if rtt_exec_path:
            lines.append('PATH="${RTT_EXEC_PATH}:$PATH"')
        if env_tools_dir:
            lines.append('PATH="${ENV_TOOLS_DIR}:$PATH"')
        lines.append('FLASH_ADDR="0x90000000"')
        lines.append('BIN_FILE="rtthread.bin"')
        lines.append('STLDR="$(dirname "$0")/board/stldr/ART-Pi_W25Q64.stldr"')
